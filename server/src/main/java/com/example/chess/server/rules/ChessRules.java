package com.example.chess.server.rules;
...
public class ChessRules {
    ...
    public boolean isLegalMoveForPiece(Board board, char piece, Move m, boolean isWhite) {
        ...
        return switch (p) {
            ...
            case 'n' -> adx * adx + ady * ady == 5;          // knight L
            case 'b' -> adx == ady && isPathClear(board, m.fromRow, m.fromCol, m.toRow, m.toCol);
            case 'r' -> (dx == 0 || dy == 0) && isPathClear(board, m.fromRow, m.fromCol, m.toRow, m.toCol);
            case 'q' -> ((dx == 0 || dy == 0 || adx == ady) &&
                    isPathClear(board, m.fromRow, m.fromCol, m.toRow, m.toCol));
            case 'k' -> adx <= 1 && ady <= 1;
            default -> false;
        };
    }

    public boolean isLegalPawnMove(Board board, Move m, boolean isWhite) {
        int dir = isWhite ? -1 : 1;
        ...
        if (dx == 0) {
            ...
            if (m.fromRow == startRow && dy == 2 * dir) {
                ...
                return dest == '.' && board.get(midRow, m.fromCol) == '.';
            }
            return false;
        }
    ...
    public boolean isPathClear(Board board, int fromRow, int fromCol, int toRow, int toCol) {
    ...
    public boolean isKingInCheck(Board board, boolean isWhite) {
        ...
        if (kr == -1) return true; // no king = effectively check
    ...
    public boolean hasAnyLegalMove(Board board, boolean forWhite) {
        for (int fromRow = 0; fromRow < 8; fromRow++) {
            for (int fromCol = 0; fromCol < 8; fromCol++) {
                ...
                if (Character.isUpperCase(piece) != forWhite) continue;
